...
Executable::Executable(const std::string &name, std::string description, const std::vector<std::string> &requiredResourcesNames, int durationUnits)
    : name(name),
      ...
      assignedResources() {
    if (name.empty())
        throw std::invalid_argument("Executable::Executable(): name is empty");

    if (durationUnits <= 0)
        throw std::invalid_argument("Executable::Executable(): durationUnits must be positive");
}

std:: string Executable::getName() const { return name; }
...
void Executable::assignResource(
    ...
    for (const auto &resourceName : requiredResourcesNames) {
        ...
        if (!found) {
            releaseResources();
            throw std::runtime_error("Resource " + resourceName + "not found or not available for" + name + "executable");
        }
    }
}
...
void Executable::releaseResources() {
    for (auto &resource : assignedResources) {
        try {
            ...
            std::cerr << "Warning: Failed to release resource " << resource->getName() << " for executable " << name << ": " << e.what() << std::endl;
